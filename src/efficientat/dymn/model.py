...
from src.efficientat.dymn.dy_block import DynamicInvertedResidualConfig, DY_Block, DynamicConv, DyReLUB
from src.efficientat.mn.block_types import InvertedResidualConfig, InvertedResidual
...
class DyMN(nn.Module):
    ...
    def _feature_forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        features = []
        x = self.in_c(x)
        if self.out_indices is not None and 0 in self.out_indices:
            features.append(x)
        g = None
        for (i, layer) in enumerate(self.layers):
            x = layer(x)
            if self.out_indices is not None and i+1 in self.out_indices:
                features.append(x)
        x = self.out_c(x)
        if self.out_indices is not None and i+2 in self.out_indices:
            features.append(x)
        return x, features
    ...
    def _forward_impl(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        x, features = self._feature_forward(x)
        if self.features_only:
            return x, features
        else:
...
def _dymn(
    ...
    if pretrained_name:
        ...
        if model.features_only:
            print("Features only is true, Dropping final fully-connected layer and loading weights in non-strict mode")
